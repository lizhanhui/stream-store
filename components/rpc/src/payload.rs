...
/// Build a client_seal payload for client→StreamManager: [extent_id:u32]
///
/// Wire size: 4 bytes fixed.
/// Used when the caller does not know the committed offset.
/// SM will query all EN replicas to determine offset via quorum.
pub fn build_client_seal_payload(extent_id: u32) -> Bytes {
    let mut buf = BytesMut::with_capacity(4);
    buf.put_u32(extent_id);
    buf.freeze()
}

/// Parse a client_seal payload: [extent_id:u32]
///
/// Returns `extent_id`.
pub fn parse_client_seal_payload(payload: &[u8]) -> Option<u32> {
    if payload.len() != 4 {
        return None;
    }
...
/// Build an extent_node_seal payload for EN→StreamManager: [extent_id:u32][offset:u64]
///
/// Wire size: 12 bytes fixed.
/// Used when the primary ExtentNode proactively seals (e.g. ExtentFull).
/// `offset` = base_offset + message_count (the next writable offset after the last committed message).
/// SM trusts this offset and skips querying EN replicas.
pub fn build_extent_node_seal_payload(extent_id: u32, offset: u64) -> Bytes {
...
/// Parse an extent_node_seal payload: [extent_id:u32][offset:u64]
///
/// Returns `(extent_id, offset)`.
pub fn parse_extent_node_seal_payload(payload: &[u8]) -> Option<(u32, u64)> {
    if payload.len() != 12 {
        return None;
    }
    let extent_id = u32::from_be_bytes(payload[0..4].try_into().ok()?);
    let offset = u64::from_be_bytes(payload[4..12].try_into().ok()?);
    Some((extent_id, offset))
}

// ── Management API payloads ──

/// Build a DescribeStream request payload: [count:u32]
pub fn build_describe_stream_payload(count: u32) -> Bytes {
    let mut buf = BytesMut::with_capacity(4);
    buf.put_u32(count);
    buf.freeze()
}

/// Parse a DescribeStream request payload: [count:u32]
pub fn parse_describe_stream_payload(payload: &[u8]) -> Option<u32> {
    if payload.len() < 4 {
        return None;
    }
    Some(u32::from_be_bytes(payload[0..4].try_into().ok()?))
...
/// Build a DescribeExtent request payload: [extent_id:u32]
pub fn build_describe_extent_payload(extent_id: u32) -> Bytes {
    let mut buf = BytesMut::with_capacity(4);
    buf.put_u32(extent_id);
    buf.freeze()
}

/// Parse a DescribeExtent request payload: [extent_id:u32]
pub fn parse_describe_extent_payload(payload: &[u8]) -> Option<u32> {
    if payload.len() < 4 {
        return None;
    }
    Some(u32::from_be_bytes(payload[0..4].try_into().ok()?))
...
pub fn encode_extent_info_vec(extents: &[ExtentInfo]) -> Bytes {
    ...
    for ext in extents {
        ...
        buf.put_u64(ext.base_offset);
        buf.put_u32(ext.message_count);
...
pub fn parse_extent_info_vec(payload: &[u8]) -> Option<Vec<ExtentInfo>> {
    ...
    for _ in 0..num_extents {
        // Need at least 4+8+4+1+2 = 19 bytes for extent header
        if payload.len() < pos + 19 {
            return None;
        }
        let extent_id = u32::from_be_bytes(payload[pos..pos + 4].try_into().ok()?);
        pos += 4;
        let base_offset = u64::from_be_bytes(payload[pos..pos + 8].try_into().ok()?);
        pos += 8;
        let message_count = u32::from_be_bytes(payload[pos..pos + 4].try_into().ok()?);
        pos += 4;
...
mod tests {
    ...
    #[test]
    fn client_seal_payload_roundtrip() {
        let payload = build_client_seal_payload(42);
        assert_eq!(payload.len(), 4);
        let extent_id = parse_client_seal_payload(&payload).unwrap();
        assert_eq!(extent_id, 42);
    }

    #[test]
    fn client_seal_payload_wrong_size() {
        assert!(parse_client_seal_payload(&[]).is_none());
        assert!(parse_client_seal_payload(&[0u8; 3]).is_none());
        assert!(parse_client_seal_payload(&[0u8; 5]).is_none());
        // Exactly 4 bytes should work
        let payload = build_client_seal_payload(1);
        assert!(parse_client_seal_payload(&payload).is_some());
    }

    #[test]
    fn extent_node_seal_payload_roundtrip() {
        let payload = build_extent_node_seal_payload(42, 123456789);
        assert_eq!(payload.len(), 12);
        let (extent_id, offset) = parse_extent_node_seal_payload(&payload).unwrap();
        assert_eq!(extent_id, 42);
        assert_eq!(offset, 123456789);
    }

    #[test]
    fn extent_node_seal_payload_wrong_size() {
        assert!(parse_extent_node_seal_payload(&[]).is_none());
        assert!(parse_extent_node_seal_payload(&[0u8; 11]).is_none());
        assert!(parse_extent_node_seal_payload(&[0u8; 13]).is_none());
        // Exactly 12 bytes should work
        let payload = build_extent_node_seal_payload(1, 2);
        assert!(parse_extent_node_seal_payload(&payload).is_some());
    }

    // -- Management API payload tests --

    #[test]
    fn describe_stream_payload_roundtrip() {
        let payload = build_describe_stream_payload(5);
        let count = parse_describe_stream_payload(&payload).unwrap();
        assert_eq!(count, 5);

        // count=0 means "all"
        let payload = build_describe_stream_payload(0);
        let count = parse_describe_stream_payload(&payload).unwrap();
        assert_eq!(count, 0);
    }

    #[test]
    fn describe_stream_payload_too_short() {
        assert!(parse_describe_stream_payload(&[]).is_none());
        assert!(parse_describe_stream_payload(&[0x00, 0x00, 0x00]).is_none());
    }

    #[test]
    fn describe_extent_payload_roundtrip() {
        let payload = build_describe_extent_payload(42);
        let extent_id = parse_describe_extent_payload(&payload).unwrap();
        assert_eq!(extent_id, 42);
    }

    #[test]
    fn describe_extent_payload_too_short() {
        assert!(parse_describe_extent_payload(&[]).is_none());
        assert!(parse_describe_extent_payload(&[0u8; 3]).is_none());
    }
