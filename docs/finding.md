...
**Resolution:** `handle_epoch_seal` now requires the request epoch to equal both the persisted epoch and the active extent's epoch before contacting an Extent Node, so stale and future requests fail without sealing a successor extent. The epoch is then advanced only inside `seal_and_allocate_transaction`, which locks the stream row and refuses to proceed unless the persisted epoch still equals the request epoch (see finding 6). Of concurrent requests for the same epoch, exactly one can advance it; losers receive `EpochStale` with their rejected request epoch and must rediscover or retry.

The database fence alone did not cover the primary-unreachable fallback, which rereads the active extent and can observe the winner's freshly allocated successor. Because `resolve_committed_offset` physically seals the extent on every replica before the transaction runs, a losing request could seal the winner's active extent and only then learn its epoch was stale. `seal_extent` therefore compares the active extent's epoch against the caller's expected epoch and fails with `EpochStale` before issuing any Extent Node seal. `reconcile_extents` is fenced the same way — it locks the stream row and rejects reports for an epoch the stream has already left, rather than writing extent rows and advancing `stream_sequence` for an abandoned epoch.

### 6. Epoch bump and successor allocation are separate transactions — Resolved

Previously the stream epoch was committed by a standalone `bump_epoch` before `seal_allocate_register` ran its own transaction over the seal, successor insertion, and replica rows. An allocation or database failure after the bump left `stream.epoch` advanced while the active extent and replica set stayed in the old epoch, conflicting with the single transition described in `design.md:38-43`, `design.md:60`, and `design.md:85`. Once finding 5 tightened `handle_epoch_seal` to require the request epoch to match both values, that divergence became unrecoverable: no epoch could satisfy both checks, and neither the heartbeat checker nor startup reconciliation repairs the mismatch.

**Resolution:** `seal_and_allocate_transaction` now performs the whole transition. It locks the stream row with `SELECT ... FOR UPDATE`, fences the caller's expected epoch, then seals the extent, inserts the successor and its replica rows at `expected + 1`, and advances `stream.epoch` — all in one transaction. Node selection and stream-config reads happen before the transaction and mutate nothing, so any failure leaves the stream wholly at the requested epoch. The standalone `bump_epoch` was removed so the unsafe ordering cannot be reintroduced. When an Extent Node has already sealed the extent autonomously and a successor exists, the transaction returns `AlreadySealed` and leaves the epoch untouched, preserving the invariant that `stream.epoch` equals the active extent's epoch. That idempotency check treats every non-`Active` state as sealed, `Flushed` included: a predecessor that reached S3 before the seal request landed would otherwise fall through to allocation and leave the stream with two active extents — the Extent Node's successor and the transaction's — with the epoch bumped past both. The repair that advances a sealed extent's recorded end offset also runs inside this transaction, under the epoch fence; it previously ran as an unconditional statement outside the transaction, where it could regress a flushed extent back to `Sealed`. The repair is bounded on both sides: it never moves the boundary backwards, never moves it past the offset where the next extent already starts, and never touches a `Flushed` extent at all. Without those bounds a delayed retry reporting a further offset would stretch the predecessor across its successor — making the same offsets readable from two extents — and, for a flushed extent, advertise bytes that were never uploaded to S3. `MetadataStore::seal_extent` was removed so no seal can be committed outside the transition.

The successor reported by `AlreadySealed` is the *active* one at the stream's own epoch. A sealed successor is not a usable answer because the caller hands that extent to a client as the new append target, and neither is an active row stranded at another epoch, which clients can no longer append to. Exactly one row should qualify; metadata written before this transaction became atomic can contain more, so the lowest extent id wins (it continues the offset chain) and the rest are logged. If nothing is taking writes, the transaction allocates a new active extent starting past the furthest sealed boundary so it cannot overlap a sealed successor, and `SealResult::Sealed` reports that offset so the caller registers the extent on the Extent Node with the value MySQL persisted rather than recomputing it from the sealed extent's end. Both seal paths in `handle_epoch_seal` pass the client's request epoch and report the epoch the transaction settled on. MySQL-backed tests in `tests/epoch_cas_integration.rs` cover concurrent transitions from the same epoch (one `Sealed` at the next epoch, one `EpochStale`, one successor extent, replica rows present), a flushed predecessor returning the existing successor without allocating a second active extent or advertising bytes past what reached S3, a delayed seal that cannot stretch a predecessor across its successor's start, a successor allocated past a sealed chain reporting the start offset it persisted, and a transaction that fails on the successor insert, asserting that the seal, the `stream_sequence` bump, the epoch, and the replica rows all roll back.
